...
Monochrome is deliberate -- one fill color + a good density ramp + high
contrast reads as neat and legible. Each row is revealed with a left-to-right
clip wipe plus a small block cursor riding the wipe edge, staggered top ->
bottom, so the whole portrait prints once and freezes.

Input:  source-prepped.png  (produced by prep_photo.py — background removed,
        CLAHE local contrast applied, composited onto white)
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "raj-ascii.svg")
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'raj@github:~$ whoami <tspan fill="{INK}">Raj Patil</tspan></text>')
# blinking cursor — positioned after "Raj Patil" (roughly 10 chars * ~7.5px + "raj@github:~$ whoami " padding)
