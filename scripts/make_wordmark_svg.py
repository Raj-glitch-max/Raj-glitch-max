"""
Render "RAJ" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# blank rows above and below the art. 5 pads the panel out to match portrait height.
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 5))
CELL_W = 9.0
CELL_H = 15.5

# Font: prefer WORDMARK_FONT env var (CI can set this), then check common Linux
# paths in order. DejaVu Sans Bold gives good weight + readable counters at
# this grid resolution. A very heavy face (Impact) collapses to blobs.
...
_MACOS_FONT = "/System/Library/Fonts/Futura.ttc"


def _find_font():
    forced = os.environ.get("WORDMARK_FONT")
    if forced and os.path.exists(forced):
        return forced, int(os.environ.get("WORDMARK_FONT_INDEX", 0))
    if os.path.exists(_MACOS_FONT):
        return _MACOS_FONT, int(os.environ.get("WORDMARK_FONT_INDEX", 2))
    for p in _LINUX_FONT_CANDIDATES:
        if os.path.exists(p):
            return p, 0
    # Last resort: PIL's built-in bitmap font (no TrueType needed, lower quality)
    return None, 0


FONT_PATH, FONT_INDEX = _find_font()

TEXT = os.environ.get("WORDMARK_TEXT", "RAJ")

MASK_H = 300           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.14        # extra letter-spacing, in em
...
def build_shell():
    ...
    if FONT_PATH is not None:
        for _ in range(40):                       # shrink until one line fits
            try:
    ...
    def line_w(s):
        try:
            ...
            return sum(font.getsize(c)[0] for c in s) + track * (len(s) - 1)
        ...
        try:
            ...
            for ch in s:
                ...
                try:
                    ...
                    pen += font.getsize(ch)[0] + track
        except Exception:
            d.text((4, 4 + li * line_h), s, font=font, fill=255)

    mask = np.array(img) > 127
    xs_any = np.nonzero(mask.any(0))[0]
    ys_any = np.nonzero(mask.any(1))[0]
    if len(xs_any) == 0 or len(ys_any) == 0:
        raise RuntimeError("Empty mask — font could not render TEXT. "
                           "Set WORDMARK_FONT to a valid bold TTF path.")
...
def fit(projected):
    """Width-driven scale + offset. Returns row count the art needs."""
    global ROWS
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">raj@github: ~$ ./wordmark.sh --3d</text>')
